...
add address=10.255.2.5/30 interface=ether5 comment="R1-REN"
...
# Inbound policy is applied on R1.  Empty communities retain RouterOS's
# default local preference; exact lab community sets receive explicit values.
...
/routing/filter/rule
add chain=bgp-in-isp rule="if (dst in 10.64.0.0/10 && dst-len >= 16 && dst-len <= 24 && bgp-communities-empty) { accept }"
add chain=bgp-in-isp rule="if (dst in 10.64.0.0/10 && dst-len >= 16 && dst-len <= 24 && bgp-communities equal-list community-isp-adjacent-origin) { set bgp-local-pref 200; accept }"
add chain=bgp-in-isp rule="if (dst in 10.64.0.0/10 && dst-len >= 16 && dst-len <= 24 && bgp-communities equal-list community-isp-transit-learned) { set bgp-local-pref 160; accept }"
add chain=bgp-in-isp rule="reject"
add chain=bgp-in-ren rule="if (dst in 10.64.0.0/10 && dst-len >= 16 && dst-len <= 24 && bgp-communities-empty) { accept }"
add chain=bgp-in-ren rule="if (dst in 10.64.0.0/10 && dst-len >= 16 && dst-len <= 24 && bgp-communities equal-list community-ren-adjacent-origin) { set bgp-local-pref 200; accept }"
add chain=bgp-in-ren rule="if (dst in 10.64.0.0/10 && dst-len >= 16 && dst-len <= 24 && bgp-communities equal-list community-ren-transit-learned) { set bgp-local-pref 180; accept }"
add chain=bgp-in-ren rule="reject"
add chain=bgp-out-campus rule="if (dst == 10.255.10.0/24 || dst == 10.255.20.0/24) { accept } else { reject }"

/routing bgp connection
add name=to-isp instance=campus-bgp remote.address=10.255.2.2 remote.as=65001 local.address=10.255.2.1 local.role=ebgp tcp-md5-key="campus-isp-ebgp" connect=yes listen=yes hold-time=30s afi=ip input.filter=bgp-in-isp input.limit-process-routes-ipv4=1000 output.redistribute=ospf output.filter-chain=bgp-out-campus
add name=to-ren instance=campus-bgp remote.address=10.255.2.6 remote.as=65002 local.address=10.255.2.5 local.role=ebgp tcp-md5-key="campus-ren-ebgp" connect=yes listen=yes hold-time=30s afi=ip input.filter=bgp-in-ren input.limit-process-routes-ipv4=1000 output.redistribute=ospf output.filter-chain=bgp-out-campus

# Track the ISP BGP FSM directly; REN does not qualify as Internet default.
/system script
