...
add name=campus-ospf version=2 router-id=10.255.0.1 redistribute=bgp out-filter-chain=ospf-external
/routing ospf area
...
# Only generated control-plane routes enter OSPF, as OSPF external type 1.
/routing filter rule
add chain=ospf-external rule="if (protocol bgp && dst in 10.64.0.0/10 && dst-len >= 16 && dst-len <= 24) { set ospf-ext-type type1; set ospf-ext-metric 20; accept } else { reject }"
